...
Doing a lot with Mistral on this on [2026-03-16 Mon]. Doesn't quite work yet.
...
Here is the additional code that gets us close and makes the change (not using current-time) work, but the days are off:

#+begin_src elisp :tangle yes


(defun mrl/get-agenda-date ()
  "Return the date at the top of the agenda buffer as YYYY-MM-DD.
Assumes the agenda buffer is current and the date is on the second line."
  (interactive)
  (save-excursion
    (goto-char (point-min))
    (forward-line 1)
    (beginning-of-line)
    (when (looking-at "\\([A-Za-z]+\\)[ \t]+\\([0-9]+\\)[ \t]+\\([A-Za-z]+\\)[ \t]+\\([0-9]+\\)")
      (let* ((day (string-to-number (match-string 2)))
             (month-name (match-string 3))
             (year (string-to-number (match-string 4)))
             (month-num (cdr (assoc-string month-name
                                           '(("January" . 1) ("February" . 2) ("March" . 3) ("April" . 4)
                                             ("May" . 5) ("June" . 6) ("July" . 7) ("August" . 8)
                                             ("September" . 9) ("October" . 10) ("November" . 11) ("December" . 12))
                                           t))))
        (message "Parsed: day=%d, month=%d, year=%d" day month-num year)
        (format-time-string "%Y-%m-%d" (encode-time 0 0 0 day month-num year))))))

(defvar mrl-current-agenda-date nil)

(defun mrl/set-agenda-date ()
  (when (get-buffer "*Org Agenda*")
    (with-current-buffer "*Org Agenda*"
      (setq mrl-current-agenda-date (mrl/get-agenda-date)))))

(add-hook 'org-agenda-finalize-hook 'mrl/set-agenda-date)
